...
def donor_dashboard_view(request):
    ...
    dict={
        'requestpending': bmodels.RequestBllod.objects.all().filter(donner_request=donor).filter(status='Pending').count(),
        'requestapproved': bmodels.RequestBllod.objects.all().filter(donner_request=donor).filter(status='Approved').count(),
        'requestmade': bmodels.RequestBllod.objects.all().filter(donner_request=donor).count(),
        'requestrejected': bmodels.RequestBllod.objects.all().filter(donner_request=donor).filter(status='Rejected').count(),
    }
...
def make_request_view(request):
    ...
    if request.method=='POST':
        ...
        if request_form.is_valid():
            ...
            donor= models.Donor.objects.get(user_id=request.user.id)
            blood_request.donner_request=donor
            blood_request.save()
...
def request_history_view(request):
    donor= models.Donor.objects.get(user_id=request.user.id)
    blood_request=bmodels.RequestBllod.objects.all().filter(donner_request=donor)
